...
export default function RagAgentPage() {
  ...
  async function askAgent() {
    ...
    try {
      ...
      if (!data.success) {
        setAnswer(data.error || "Erro ao consultar o agente.");
        return;
      }
      ...
      setAnswer("Erro ao comunicar com o RAG Agent.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        <div className="mb-8 rounded-3xl border border-cyan-400/20 bg-slate-900/70 p-8">
          ...
          <h1 className="text-4xl font-black tracking-tight">
            Agente Especialista com Memória Técnica
          </h1>

          <p className="mt-5 max-w-3xl text-base leading-7 text-slate-300">
            Consulte a memória técnica do EQUATEC com recuperação contextual,
            análise executiva e recomendações estruturadas por IA.
          </p>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
          <section className="rounded-3xl border border-slate-800 bg-slate-900 p-6">
            ...
            <textarea
              ...
              placeholder="Ex.: Quais riscos técnicos e SSMA aparecem em documentos similares de manutenção industrial?"
            ...
            >
              {loading ? "Consultando agente..." : "Perguntar ao agente"}
            </button>

            <div className="mt-5 rounded-2xl border border-cyan-400/20 bg-cyan-400/10 p-4">
              <p className="text-sm font-bold text-cyan-200">
                Contextos técnicos recuperados
              </p>
            ...
            <div className="min-h-[680px] rounded-2xl border border-slate-800 bg-slate-950 p-5">
              {answer ? (
                ...
                <div className="flex h-full items-center justify-center text-center text-sm leading-6 text-slate-500">
                  Faça uma pergunta para o agente consultar a memória técnica.
                </div>
              )}
            </div>
          </section>
